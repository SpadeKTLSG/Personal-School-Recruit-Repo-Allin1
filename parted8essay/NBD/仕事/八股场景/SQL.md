...
对于更新 JSON 数据中的部分字段，可以使用 MySQL 提供的 JSON 函数，如 `JSON_SET`​、`JSON_REPLACE`​ 和 `JSON_REMOVE`​
...
#### server层: 缓存：为什么新版本删掉了

* **并发瓶颈**：在高并发场景下，查询缓存**存在全局锁**，容易成为性能瓶颈。
* **低命中率和维护成本高**：对于频繁更新的表，缓存失效率较高，缓存管理和刷新开销大，效果反而不如预期。
* **多核时代不适用**：全局锁在多核处理器上会极大限制并发执行的能力。

‍

为了整体提升并发性能和简化架构，新版本选择移除查询缓存，将更多精力放在提高存储引擎和缓存层（如操作系统缓存）的性能上

‍

‍

#### server层: 优化器：原理，怎么判断最优

MySQL 的查询优化器是一个**基于成本的优化器（CBO）** ，其核心工作流程和判断依据包括：

* **成本估算**：优化器会为每个可能的执行计划计算一个“成本”，主要依据包括I/O代价、CPU代价、内存消耗以及数据量、索引选择性等统计信息。
* **执行计划枚举**：针对同一SQL语句，优化器会尝试多种不同的访问路径和连接顺序，如使用索引、全表扫描、不同的连接算法（嵌套循环、合并连接、哈希连接）等。
* **最优选择**：最终选择成本最小的执行计划。优化器在选择过程中依赖统计信息（如表的行数、索引基数、数据分布等）来评估每种方案的代价。

这种基于成本的优化方式允许MySQL根据当前数据情况和硬件资源动态选择最优执行计划

‍

‍

#### engine层: innodb结构，插入一条数据的流程（buffer pool、日志的写时机、树中的维护）

在InnoDB引擎中，插入数据大致经历以下几个步骤

‍

1. **内存分配与Buffer Pool操作**

    * 插入操作首先在内存中的Buffer Pool中查找目标页。
    * 如果目标页空间足够，则直接将新记录写入内存页；如果空间不足，则可能触发页分裂操作。
2. **Redo Log（重做日志）的写入**

    * 插入操作会在Buffer Pool中修改数据，同时将变更记录写入Redo Log缓冲区。
    * 在事务提交或达到一定条件后，这些日志会刷写到磁盘，以保证事务的持久性（例如通过Group Commit机制）。
3. **B+树的维护**

    * InnoDB使用B+树作为聚集索引，新记录的插入需要找到正确的叶子节点位置。
    * 如果叶子节点空间不足，新记录的插入可能导致页分裂，并相应调整父节点指针，维护B+树结构。
4. **脏页刷新与CheckPoint**

    * Buffer Pool中的脏页（已修改但尚未写入磁盘的页）会在后台被刷新到磁盘，确保数据持久性，并降低恢复时间窗口。

‍

‍

#### engine层: b+树复杂度，跳表的复杂度，为什么不用跳表

‍

**复杂度比较**：

* **B+树**：查找、插入、删除的时间复杂度是 O(logₘN)，其中 m 表示树的阶（即每个节点最多的子节点数，通常较大，如几十甚至上百），这意味着树的高度通常很低。
* **跳表**：平均时间复杂度为 O(log₂N)，虽然也是对数级别，但底数为2，相对于 B+树较低的分支因子，树高可能会更高，从而导致更多的I/O操作。

‍

都是logN，但b+树的底为m，跳表的底为2

‍

**为何不用跳表**

1. **I/O效率**：B+树由于分支因子大，能在更少的层级内定位数据，从而减少磁盘I/O次数
2. **易维护性**：B+树在数据库系统中成熟稳定，支持范围查询、顺序扫描，并且在插入和删除时表现更稳定。
3. **事务支持**：B+树结构更适合与InnoDB的**事务**及锁机制配合，确保数据的一致性和隔离性。

‍

‍

#### engine层: 为什么索引越小，性能越好

‍

**存储密度高**

索引数据越小，每个磁盘页可以存放更多索引记录，这样树的高度会降低，从而减少查找时需要读取的页数

‍

**I/O次数减少**

较小的索引能更有效地利用缓存（如Buffer Pool），减少磁盘I/O，提高查询速度

‍

**传输和处理效率**

在执行查询时，索引数据需要从磁盘读取到内存，数据越小，数据传输时间和处理时间都会降低

‍

‍

### MySQL最多可以存多少数据？为什么有人推荐是2000W而不是2亿？

‍

**理论存储上限：**   
MySQL本身对单表存储的数据量没有严格的限制，实际上限取决于操作系统、文件系统、硬件配置以及存储引擎的实现。例如，InnoDB单表在64位系统上可以达到TB级别（如64TB）的存储上限。
...
* **索引效率：**   
  当单表记录数在2000W左右时，B+树索引结构通常层级较少（一般2\~3层），查找时磁盘I/O次数较少，整体查询性能较高。如果数据量达到2亿级别，索引树可能会加深，导致查询和插入性能下降。
* **维护成本：**   
  数据量越大，备份、恢复、重建索引、表扫描等操作的开销也越大。2000W是一种较为平衡的规模，既能满足大部分业务需求，又不会因数据量过大而使维护工作变得异常繁重。
* **系统资源和稳定性：**   
  超大表的数据操作对内存、磁盘I/O、缓冲池等都有更高要求，超过2000W后需要更复杂的分库分表策略来保持系统的响应速度和稳定性。

‍

**为什么不是2亿？**   
虽然MySQL理论上可以存储更大量的数据，但实际应用中：

* 超过2000W的数据行，会显著增加索引维护、查询优化和系统负载的难度。
* 数据库性能、备份恢复效率以及扩展性方面，都需要额外的优化手段（如分区、分库分表等），因此很多生产环境会将单表记录数控制在2000W左右，避免性能瓶颈。
...
MySQL 原生引擎，但它并不支持事务功能，这也是后来被 InnoDB 替代为默认引擎的主要原因。

MyISAM 有独立的索引文件，因此在**读取数据**方面的性能很高，它也支持全文索引、地理位置存储和索引等功能，但==不支持外键==
...
2. **InnoDB缓冲池（InnoDB Buffer Pool）** ：
    ...
    * InnoDB缓冲池是MySQL的第二级缓存，用于缓存表数据和索引。它是InnoDB存储引擎的核心组件，负责将磁盘上的数据页缓存到内存中，以提高数据访问速度。 (可以设置来提升性能, 能提升)
...
但是大部分由于要做到数据库载体的解耦, 不能使用这些特性. 除非确定未来都是用这个库. 要使用DO -> BO 的Tmodel实现
...
1. **性能问题**：`TEXT`​类型的数据存储和检索性能较差，因为它们存储在一个**单独的区域**，而不是行内存储。每次访问`TEXT`​字段时，都会有额外的I/O开销。
2. **索引限制**：`TEXT`​类型的字段**不能直接创建索引**，必须指定前缀长度，这会限制索引的有效性和性能。
...
### 有张表用uuid建表，另张表用自增id建表，1kw行记录，添加数据的效率谁更高？为什么？谁的查询效率高？为什么？
...
2. **页分裂**：自增ID不会导致页分裂，而UUID由于是随机的，可能会导致频繁的**页分裂**，影响插入性能。
...
* **数据存储**：在InnoDB中，主键索引的叶子节点存储的是整行数据，因此主键索引也被称为**聚簇索引**。
...
* **查询过程**：当使用非主键进行查询时，InnoDB引擎首先通过非主键索引找到对应的**主键值**，然后再通过主键索引找到整行数据。这种过程称为 **“回表”** 。由于需要进行两次查找，查询效率相对较低。
...
### MySQL中主键"id"字段乱序的话 (不采用自增主键, 而是用了分布式的UUID等), 影响我们去读数据库还是写数据库的性能

使用乱序主键（比如使用分布式UUID而非自增）通常会主要影响写性能

‍

* **聚集索引失序**  
  InnoDB存储数据时以聚集索引（主键索引）为顺序。如果主键值是随机的，每次插入新记录可能需要在B+Tree中找到非尾部的位置，这会导致频繁的**页分裂**和页间重排，从而增加写入时的随机I/O开销。
* **页分裂问题**  
  随机插入会使得原本连续的空间被打乱，导致页分裂、索引碎片增加，降低写操作效率。
* **读性能间接受影响**  
  虽然主要问题在写，但频繁的页分裂和碎片化可能使索引层级加深，间接增加查询的成本，不过总体来看写性能受到的冲击更明显。

‍

因此，如果主键值乱序，最显著的问题是写入效率降低，而读操作受影响则相对较小

‍

‍

### 如果有两个字段age和name做联合索引，怎么安排更好? 不给出其他条件, 请尽可能进行设想

‍

1. **索引前缀原则**

    * 联合索引可以用最左前缀原则（即只要查询条件中包含索引的最左边字段，就能利用索引）
2. **字段的选择性**

    * 选择性高的字段（不同值多，过滤效果好）应放在前面。
    * 通常来说，age 是数值类型且取值范围有限（比如只有几十个不同值），而 name 一般字符串且可能具有较高的离散性。
    * 如果 name 的选择性更高，那么把 name 放在前面能使得索引过滤更精准，从而提高查询性能
3. **查询条件的构成**

    * 如果查询经常同时使用 age 和 name 作为条件，例如 `WHERE name = 'Alice' AND age = 20`​，那么无论索引顺序如何，两字段都能被利用；
    * 但如果有很多查询仅根据 name 进行筛选，那么把 name 放在前面（即联合索引为 name, age）会更有优势；
    * 反之，如果查询常常只根据 age 查询，则应考虑把 age 放前面，但由于 age 的基数较小，这样的查询可能选择性不高。
4. **覆盖查询和索引大小**

    * 考虑到索引记录的大小和IO效率，字段较小的数值型数据（如 age）通常更节省空间。如果联合索引需要覆盖更多查询（例如查询只返回这两个字段），则需要平衡字段顺序与覆盖效果。
...
* 如果假设数据中 name 的**唯一性或离散性明显高于 age**，而且预计大部分查询都会涉及 name（例如查询某个名字的记录），那么建议使用  **(name, age)**  的顺序。这样既能支持只按 name 查询，也能支持联合查询。
* 反过来，如果查询场景中大多数都是根据 age 进行范围查询（例如 age 在某个区间内），而 name 只是附带信息，则可以考虑  **(age, name)** ，这样可以直接利用 age 的范围查找。不过，由于 age 的选择性一般较低，这种情况下索引效果可能不如前者。

‍

所以，在没有其他条件的情况下，更倾向于把 **高选择性、较为分散的字段（通常是 name）放在前面**，即采用 (name, age) 的联合索引顺序。但具体选择还应依据实际查询模式、数据分布和业务需求来决定。

‍

‍

### MySQL在表上新增字段时如何保证不影响读写操作

* **在线DDL**

  * 利用 MySQL 5.6 及以上版本支持的在线 DDL（ALTER TABLE ... ADD COLUMN ... ALGORITHM\=INPLACE）在大多数情况下可以在线修改结构而不会阻塞读写操作。
* **第三方工具**

  * 使用 pt-online-schema-change 或 gh-ost 工具，通过创建影子表、数据同步和重命名表的方式，实现零停机的表结构变更。
* **注意事项**

  * 尽量让新字段允许 NULL 或设置默认值，避免长时间的数据回填。
  * 操作前做好充分备份，尽量在业务低峰期操作。
...
* 不能完全按照数据库三大范式设计。数据库范式就是让一个字段干一个字段的事，一个表干一个表的事，拒绝冗余。但如果完全没有冗余的话，很多时候会引起**多表join**降低性能，适当增加冗余可以提高查询性能。比如要查询班级里学生的姓名，如果在班级表里冗余学生字段的话就不用去join查询学生表了，一般我们冗余的事不太容易发生变化的字段比如姓名性别等；
* 当不可避免需要join的时候，可以设计个**中间表**去存储所有数据，用了中间表就要考虑到这张表的更新机制避免数据不一致的情况（最好定期轮询检查）
* 要设计一个游标字段（自增主键和时间戳很适合当游标字段），减少limit查询(limit n,m)尽量依靠游标来查，比如说你用主键，下次查询就>或<上次查询的最后一个id就可以了 (这个就是深分页解决的方法之一)
...
* ROW_NUMBER()：为**每一行分配唯一的行号**，适合唯一标识需求。 (类似Oracle的ROWNUM)
...
ALTER TABLE `table_name`​ ADD XXX{}

* 普通索引    INDEX index_name ( `column`​ )
* 多列索引    INDEX index_name ( `column1`​, `column2`​, `column3`​ )
* 唯一索引    UNIQUE ( `column`​ )
* 主键索引    PRIMARY KEY ( `column`​ )
* 全文索引    FULLTEXT ( `column`​)
...
|week\_visit\_time|visit\_count|user\_type|
| -------------------------| -----------------| ---------------|
|1|20|supplier|
|1|30|brandOwner|
|2|60|supplier|
...
|week\_visit\_time|supplier|brandOwner|
| -------------------------| ----------| ------------|
|1|20|30|
|2|60|50|
...
|id|url|visit\_count|date|
| ----| -----| -----------------| ----------|
|3|[suvllian.com](http://suvllian.com/)|20|20200301|
|5|[suvllian.com](http://suvllian.com/)|30|20200301|
|7|[suvllian.com](http://suvllian.com/)|40|20200301|
|9|[suvllian.net](http://suvllian.net/)|30|20200301|
|12|[suvllian.net](http://suvllian.net/)|30|20200301|
|16|[suvllian.net](http://suvllian.net/)|40|20200301|
...
1. 原子性（Atomicity）：事务作为一个**整体**被执行 ，要么全部执行，要么全部不执行； -> undo
2. 一致性（Consistency）：在事务开始之前和事务结束以后，数据库的完整性没有被破坏。这表示写入的资料必须完全符合所有的预设规则，这包含资料的精确度、串联性以及后续数据库可以自发性地完成预定的工作；
3. 隔离性（Isolation）：多个事务并发执行时，一个事务的执行不应影响其他事务的执行，然后你可以扯到隔离级别；
4. 持久性（Durability）：一个事务一旦提交，对数据库的修改应该永久保存。 -> redo
...
2. 可重复读与幻读的关系： 在可重复读隔离级别下，对于已存在的记录，MVCC确保事务能够看到**第一次查询**时的数据状态，因此不会发生不可重复读。但是，当有新的行被插入到符合事务查询条件的范围内时，MVCC本身并不能阻止这种现象的发生，因为新插入的行对于当前事务是可见的（取决于插入事务的提交时间点和当前事务的Read View）
3. 为了减轻幻读的影响，InnoDB在执行某些范围查询时会采用 **间隙锁**（Gap Locks）或者Next-Key Locks来**锁定索引区间**，防止其他事务在这个范围内插入新的记录，但并非所有的范围查询都会自动加上这样的锁，而且这仅限于**使用基于索引的操作**，非索引字段的范围查询无法通过间隙锁完全避免幻读。
...
#### MySQL rr隔离级别为啥没有解决幻读

‍

**MVCC 的一致性视图**  
InnoDB 的 Repeatable Read 隔离级别采用了多版本并发控制（MVCC），在每个事务开始时为其建立一个数据快照。也就是说，即使其他事务在事务期间插入了新数据，当前事务通过一致性读（快照读）依然只看到事务启动时的数据快照。这从一致性角度“避免”了幻读，因为多次查询结果保持一致，但这也意味着**事务内看不到最新插入**的数据，如果业务需要获取最新结果，这种方式反而不符合预期。

‍

**锁定读与快照读的区别**

当使用 `SELECT ... FOR UPDATE`​ 时，InnoDB 会根据查询条件在索引上加锁（行锁或 next‑key 锁），防止其他事务在锁定范围内插入新记录，从而阻止幻读。

然而，如果查询条件未能充分利用索引（例如对非索引列或函数计算后的结果进行筛选），InnoDB 可能无法锁定所有可能插入新行的间隙（gap lock 失效），从而仍可能出现幻读现象

‍

‍

#### rr的 快照读 为啥也没解决幻读，举个快照读没法解决幻读例子

‍

**快照读的局限**

在 MVCC 机制下，事务进行快照读时，总是**读取事务启动时的数据版本**，不管其他事务随后如何插入或更新数据。

‍

例如：

* 事务 A 在 RR 隔离级别下开始后，执行如下查询（采用快照读）：
  ...
  得到某个结果集快照。
* 事务 B 在 A 之后插入了一条新的订单，满足条件 `amount > 100`​
* 如果事务 A 再次执行同样的查询，由于 A 仍然使用启动时的快照，它不会看到 B 插入的新记录。  
  这种情况在业务上可能被认为是“幻读”——因为用户期望两次查询结果一致，但实际上最新的数据未被看到。但从MVCC角度看，这是设计所决定的“一致性快照”，而非传统意义上“幻读”的并发问题。
...
如果需要保证查询能捕捉到其他事务插入的新行，就不能使用快照读，而必须采用锁定读（如 `SELECT ... FOR UPDATE`​），并且要求查询条件能够利用索引，从而使 InnoDB 对查询范围及其 gap 加锁，防止其他事务插入新行。
...
> 使用 `FOR UPDATE`​ 结合合适的索引，可以通过gap lock锁定查询范围，从而阻止其他事务插入新数据，真正避免幻读

‍

* **使用锁定读而非快照读**

  * 采用 `SELECT ... FOR UPDATE`​ 或 `LOCK IN SHARE MODE`​ 时，确保查询条件有合适的索引支持，使 InnoDB 能够对目标记录及其 gap 加锁，防止其他事务在锁定范围内插入新记录，从而避免幻读。
* **提高隔离级别**

  * 可以使用更严格的隔离级别（例如 SERIALIZABLE），该级别下所有查询都被视为锁定读，能彻底防止幻读。不过这会显著降低并发性能，所以只适用于对数据一致性要求极高的场景。
* **业务逻辑设计优化**

  * 重新设计业务场景，尽量使查询结果不依赖于实时最新数据。比如，可以在更新数据前主动加锁或预先查询确认数据状态，再执行后续逻辑；或者采用乐观锁配合版本号等策略，避免依赖全局锁定。
...
### MySQL什么时候锁行操作会变成锁表了, 总结下?

‍

1. **缺少有效索引**  
    如果SQL语句的WHERE条件没有利用索引（例如对非索引列或函数计算后的结果进行筛选），InnoDB需要对全表进行扫描，结果就是对表中每一行都加上行锁（或next‑key锁），这实际上等同于锁住整张表
2. **全表更新或删除**  
    当执行没有WHERE条件的UPDATE/DELETE，或条件非常宽泛以至于锁定绝大多数行时，锁住的行数接近全表，效果上就相当于表锁
3. **不恰当的锁定语句**  
    某些语句（如SELECT ... FOR UPDATE或LOCK IN SHARE MODE）如果没有利用到索引，也可能导致全表扫描并加锁，进而使得所有行都被锁定，表现为表级锁。
4. **DDL操作**  
    如ALTER TABLE、DROP TABLE、TRUNCATE TABLE等DDL语句，操作时会对整个表加锁，虽然这不是行锁升级，但从业务角度看效果一样。
5. **存储引擎特性**  
    使用MyISAM或其他只支持表级锁的存储引擎时，本身就只有表锁，而不是行锁。
...
* 表锁. 对一整张表加锁，虽然可分为读锁和写锁，但毕竟是锁住整张表，会导致并发能力下降，一般是做ddl处理时使用。

  另外表级别还有**AUTO-INC锁**:在执行插入语句时就在表级别加一个AUTO-INC锁，然后为每条待插入记录的AUTO_INCREMENT修饰的列分配递增的值，在该语句执行结束后，再把AUTO-INC锁释放掉。这样一个事务在持有AUTO-INC锁的过程中，其他事务的插入语句都要被阻塞，可以保证一个语句中分配的**递增值是连续**的
* 行锁 / 记录锁，顾名思义就是在记录上加的锁。锁住数据行，这种加锁方法比较复杂，但是由于只锁住有限的数据，对于其它数据不加限制，所以并发能力强，MySQL一般都是用行锁来处理并发事务。下面是行锁的类型：

  * 记录锁 Record Locks：当一个事务获取了一条记录的S型记录锁后，其他事务也可以继续获取该记录的S型记录锁，但不可以继续获取X型记录锁；当一个事务获取了一条记录的X型记录锁后，其他事务啥都不行了.
  * 间隙锁 Gap Locks：是为了防止插入幻影记录而提出的 (尽力规避幻读)，不允许其他事务往这条记录前面的间隙插入新记录。

    在 REPEATABLE READ 隔离级别下是可以解决幻读问题的，解决方案有两种，可以使用MVCC方案解决，也可以采用加锁方案解决，但是在使用加锁方案解决时有个大问题，就是事务在第一次执行读取操作时，那些幻影记录尚不存在，我们无法给这些幻影记录加上记录锁
  ...
  * 隐式锁：当事务需要加锁的时，如果这个锁不可能发生冲突，InnoDB会跳过加锁环节，这种机制称为**隐式锁**。隐式锁是InnoDB实现的一种延迟加锁机制，其特点是只有在可能发生冲突时才加锁，从而减少了锁的数量，提高了系统整体性能
...
悲观锁：在悲观锁机制下，事务认为在它执行期间数据一定会被其他事务修改，因此在读取数据时就立即对其加锁，阻止其他事务对同一数据进行操作，直到当前事务结束并释放锁。SELECT ... FOR UPDATE或SELECT ... LOCK IN SHARE MODE (DML语句 会触发行锁，DDL语句触发表锁)
 ...
 **（Record Lock）** ：也被称为记录锁，属于单个行记录上的锁。
 ...
 **（Next-Key Lock）** ：Record Lock+Gap Lock，锁定一个范围，包含记录本身，主要目的是为了解决幻读问题。记录锁只能锁住**已经存**在的记录，为了**避免插入新记录**，需要依赖间隙锁。
...
通过使用间隙锁和Next-Key Lock，可以防止其他事务在**当前事务读取的范围**内插入或删除数据，从而解决幻读问题
...
### MySQL select for update 场景综合三连

‍

‍

#### 看到select * from t where id = 1 for update，能想到什么 (会想到有没有这条, 还有 for update 加了锁)

‍

* **加锁含义**

  这条语句表面上是要查询 id \= 1 的记录并加上**排他锁**（X 锁），防止其他事务修改或读取时发生冲突

  如果该记录存在，会对该行加锁；如果不存在，根据隔离级别（默认 REPEATABLE READ）则会对查询范围内的“gap”（空隙）加锁，从而防止其他事务插入满足该条件的新记录（防止幻读）
* **锁粒度**  
  – InnoDB实现的是行锁（以及对应的 next‑key 锁），而“for update”会导致锁定返回的行或相关的 gap。

‍

‍

#### 如果没有这条记录，会锁什么？能插入id = 1吗？能插入id = 2吗?

‍

* **锁定情况**

  如果表中没有 id \= 1 的记录，InnoDB不会返回任何实际记录，但**仍会**基于查询条件在索引上建立**间隙锁（Gap Lock）**

  这个 gap 锁通常锁定的是“假定”该记录应该处于的位置所在的间隙（例如，在主键索引上，如果存在某些记录，该 gap 是前后记录之间的范围；若表为空，则锁定整个范围）
* **插入限制**

  **不能插入 id**  **=**  **1**：因为 id \= 1 落在已被 gap 锁定的范围内，新事务在该 gap 内插入记录会被阻塞。  

  **插入 id**  **=**  **2 是否受限**：这取决于该 gap 的范围。如果 id \= 2 落在同一 gap 内，也会被阻止；但如果表中已有记录且 gap 分界明确，可能 id \= 2 落在未锁定的区间，则可以插入

  在一个空表中执行此查询时，整个范围会被锁（空表全表扫描），因此所有插入操作（无论是 id \= 1 还是 id \= 2）都会受影响，直到事务结束。

‍

‍

#### 这个锁到底是什么玩意 ? 是怎么记录的? 存在哪里 ?

‍

* **锁的类型**

  这是 InnoDB 的**行锁**中的一种，通常是 **Next-Key Lock（下一键锁 -&gt; 临键锁）** ，它结合了记录锁和 gap 锁

  Next-key 锁既保护已存在的记录，也**锁定相邻的 gap**，从而防止幻读
* **锁的记录**

  InnoDB 在执行“for update”时，会在内部的锁表（或称为锁信息结构）中记录当前事务持有的锁，包括锁定的记录或间隙范围

  锁信息存储在 InnoDB 的内存中（如 Buffer Pool 内），由 InnoDB 内部的事务管理和锁管理模块负责维护
...
* 意向锁是 InnoDB 自动加的， 不需用户干预。 对于 UPDATE、 DELETE 和 INSERT 语句， InnoDB 会自动给涉及数据集加
* 排他锁（X)； 对于普通 SELECT 语句，InnoDB 不会加任何锁； 事务可以通过以下语句显式给记录集加共享锁或排他锁：
...
**作用**：表里的主键通常都会设置成自增的，之后可以在插入数据时，可以不指定主键的值，数据库会自动给主键赋值递增的值通过 **AUTO-INC 锁**实现的。**在插入数据时，会加一个表级别的 AUTO-INC 锁**，然后为被 AUTO\_INCREMENT 修饰的字段赋值递增的值，等插入语句执行完成后，才会把 AUTO-INC 锁释放掉。其他事务的如果要向该表插入语句都会被阻塞，从而保证插入数据时字段的值是连续递增的。
...
**改进**：InnoDB 存储引擎提供了一种**轻量级的锁**来实现自增。在插入数据的时候，会为被 AUTO\_INCREMENT 修饰的字段加上轻量级锁，**然后给该字段赋值一个自增的值，就把这个轻量级锁释放了，而不需要等待整个插入语句执行完后才释放锁**。

‍

#### AUTO-INC锁控制

设置innodb\_autoinc\_lock\_mode的系统变量，是用来控制选择用 AUTO-INC 锁。：

* 当 innodb\_autoinc\_lock\_mode \= 0，采用 AUTO-INC 锁，语句执行结束后才释放锁；
* 当 innodb\_autoinc\_lock\_mode \= 2，采用轻量级锁，申请自增主键后就释放锁，并不需要等语句执行后才释放。
...
当 innodb\_autoinc\_lock\_mode \= 2 是性能最高的方式，但是当搭配 binlog 的日志格式是 statement 一起使用的时候，在主从复制的场景中会发生**数据不一致的问题**。
...
也不建议使用**非单调**的字段作为主键，这样会造成主索引**频繁分裂 (排序啥的性能下降厉害) (当然分布式环境就不行了)**
...
> 注：我们知道一次io读写，可以获取到16K大小的资源，我们称之为读取到的数据区域为Page - 页。而我们的B树，B+树的索引结构，叶子节点上存放好多个关键字（索引值）和对应的数据，都会在**一次IO操作中被读取到缓存中**，所以在访问同一个页中的不同记录时，会在内存里操作，而不用再次进行IO操作了。除非发生了页的分裂，即要查询的行数据不在上次IO操作的换村里，才会触发新的IO操作。 --单位是**操作系统的页**, 一般就是16KB
...
4.不过，如果涉及到==大数据量==的排序、全表扫描、count之类的操作的话，还是==MyISAM==占优势些，因为**索引所占空间小**，这些操作是需要在内存中完成的。
...
UUID也是怕id被猜出来, 那既然这样就直接使用所谓的 DTO + BO + VO 多级层级来进行数据的屏蔽就好......但是更核心的点是分布式环境下的同时新增导致的冲突问题. 
...
2. 如果查询字段不在索引中可能会产生回表操作, 这会导致filesort，降低性能
3. 一定**要有过滤字段(where)** , 不然不能使用索引
...
1. 对于单键索引，尽量选择**过滤性更好**的索引（例如：手机号，邮件，身份证）
2. 在选择组合索引的时候，过滤性最好的字段在索引字段顺序中，**位置越靠前**越好。
3. 选择组合索引时，尽量包含where中**更多字段**的索引 (差别大, 筛选的数量相比就小)
...
### 添加索引时候如果程序在跑/如果生产一张表数据量级是千万级别以上的，要给这张表添加索引，你需要怎么做呢?
...
* **基本原理：**   
  当你在MySQL中建立联合索引（例如索引字段顺序为 col1, col2, col3）时，MySQL（以InnoDB为例）会将这三个字段的值拼接成一个复合键，按照字典顺序构建成一棵B+树。每个节点中的键都是一个三元组，整个索引的排序遵循“从左到右”的规则。
* **叶子节点存储：**   
  在InnoDB中，B+树的叶子节点存储了联合索引对应的所有列值以及指向实际数据行（或主键）的指针。这样，查询时如果用到了索引左前缀（例如只利用col1或(col1, col2)），则可以直接命中该索引，而不必扫描整个表。
* **示意图（简化版）：**

  ```
                ┌─────────────────────────────┐
                │    (a1, b1, c1) | ...       │  ← 内部节点：存储关键字分界值
                └─────────────────────────────┘
                         /         |         \
                        /          |          \
              ┌─────────┐     ┌─────────┐    ┌─────────┐
  ...
  每个叶子节点中的记录都是联合键（例如 (col1, col2, col3)）及对应的行指针，整个B+树保证了数据有序性和高效的范围查询。
...
有了 Online DDL 技术之后，在添加索引时，会对原本进行操作，并且允许和 DML（数据操作语言 INSERT、UPDATE、DELETE、SELECT）一起并发执行了
...
1. 更改代码，让数据开始**双写新旧数据库**
2. 更改读取代码，开始**灰度读取**新数据库数据
3. 把历史数据导入新数据库
4. 灰度下掉老数据库

预期会有数据不一致，性能问题等

> 例如使用 Oracle -> OceanBase 的情况, 部分比如测试的先走OceanBase, 逐步迁移
...
> 我就遇到过Docker里面部署这种东西内存不够导致的重启.

‍

#### 如果是异常关机或kill -9 mysql后导致表文件损坏
...
* DML 误操作语句造成数据不完整或者丢失。可以通过 flashback，美团的 myflash，也是一个不错的工具，本质都差不多，都是先解析 binlog event，然后在进行反转。把 delete 反转为insert，insert 反转为 delete，update前后 image 对调。所以必须设置binlog\_format\=row 和 binlog\_row\_image\=full，切记恢复数据的时候，应该先恢复到临时的实例，然后在恢复回主库上。
* DDL语句误操作(truncate和drop)，由于DDL语句不管 binlog\_format 是 row 还是 statement ，在 binlog 里都只记录语句，不记录 image 所以恢复起来相对要麻烦得多。只能通过全量备份+应用 binlog 的方式来恢复数据。一旦数据量比较大，那么恢复时间就特别长
...
* InnoDB 引擎内部，由于有淘汰策略，InnoDB 管理 Buffer\_Pool 使用的是改进的 LRU 算法，使用链表实现，实现上，按照 5:3 的比例把整个 LRU 链表分成了 young 区域和 old 区域。对冷数据的全扫描，影响也能做到可控制。
...
在执行子查询的时候，MYSQL创建了**临时表**，查询完毕后再删除这些临时表
...
> 参考阿里巴巴手册, 里面是说不超过三张. 当然实现业务而言, 我写过5, 6张表的...
...
需要注意的是，由于 Sort Buffer 的大小是有限的，因此在处理大表时，RowNumber 排序可能会导致内存不足的问题。此时，可以考虑使用**外部排序**的方式，即通过磁盘文件进行排序操作。
...
![image](assets/image-20250313104546-z80as17.png)​
...
数据库就是一个在磁盘上把数据按照一定顺序存储的软件。数据本身不在表里而在磁盘上，所谓的表只是数据存储空间的名字。数据库优化的底层逻辑，就是优化cpu从磁盘读写数据的时间
...
1. 改变硬件规格，更换更加高端的cpu, 频率更高的内存，把机械硬盘换成更好的固态硬盘
2. 增加mysql缓冲池的大小，mysql很多时候不会直接读写磁盘数据，会根据自己的算法通过缓冲池缓存磁盘的数据到内存中，来提高查询效率，并且会不断根据淘汰算法，来更新缓存中的冷热数据，保证缓存的命中率）
...
* 通过key和key_len检查是否命中了索引，如果本身已经添加了索引，也可以判断索引是否有失效的情况，
* 通过type字段查看sql是否有进一步的优化空间，是否存在全索引扫描或全盘扫描，
* 通过extra建议来判断，是否出现了回表的情况，如果出现了，可以尝试添加索引或修改返回字段来修复
...
* mysql本身的慢查询日志
  ...
  > 一般还是直接用运维的内容, 或者封装框架, 没有说操作环境去看日志的. 因为有多套环境, 不同的数据库配置, 因此性能也有差别. 还是需要回归环境进行验证的. 
  >
...
* 内存写满：如果一次查询的数据过多，查询的数据页并不在内存中，这时候就需要申请新的内存空间，而如果此时内存已满，就需要淘汰一部分内存数据页，如果是干净页就直接释放，如果是脏页(被修改了的)就需要flush
...
这条sql的连接的类型，性能由好到差为 NULL、system、const、eq_ref、ref、range、 index、all
...
* 因为数据是无序的，所以就需要排序。如果数据本身是有序的，那就不会再用到文件排序。而索引数据本身是有序的，我们通过建立索引来优化`order by`​语句。
...
为了回滚而记录, 原子性相关，主要**用于事务回滚和MVCC**

在事务没提交之前，Innodb会先记录更新前的数据到undo log中，回滚时利用 undo log 来进行回滚。每当进行一条记录进行操作(修改、删除、新增)时，要把回滚时需要的信息都记录到 undo log 里：原理是执行一条相反的操作。undo log 有两个参数：roll\_pointer 指针和一个 trx\_id 事务id，通过 trx\_id 可以知道该记录是被哪个事务修改的；通过 roll\_pointer 指针可以将这些 undo log 串成一个链表，形成版本链。
...
是**逻辑日志**，记录的是这个语句的原始逻辑，比如“给ID=2这一行的c字段加1 ”
...
**事务提交流程中的位置：**   
在使用InnoDB存储引擎时，MySQL采用“两阶段提交”的机制来保证数据一致性。其大致步骤如下：

1. **写入Redo Log：**  事务修改内容首先写入并刷新到redo log（重做日志），用于崩溃恢复。
2. **写入Binlog：**  紧接着，在事务正式提交前，将事务的变更信息以二进制日志（binlog）的形式同步写入磁盘（如果 sync\_binlog\=1，则为同步写入）。
3. **返回提交成功：**  此时事务提交成功，数据页刷新到磁盘的动作则可以异步进行。

‍

**为什么先写Binlog？**

* **确保数据持久性和复制一致性：**  通过在数据页尚未刷新前就把binlog写入磁盘，确保即使系统发生崩溃，也能依靠binlog进行恢复和主从复制；如果binlog延后写入，可能会导致已提交的事务没有被记录，从而引起数据不一致。
* **两阶段提交保证：**  binlog的写入是两阶段提交中的关键步骤，确保在事务真正提交前，所有需要恢复的信息已经安全持久化。

  
**binlog是在数据页实际写入磁盘前就被同步记录下来的。**  这样设计能在崩溃恢复、主从复制等场景中，保证所有已提交事务的日志信息完整无缺，降低数据丢失风险。
...
先写 logfile0 文件，当 logfile0 文件被写满的时候，会切换至 logfile1 文件，当 logfile1 文件也被写满时，会切换回 ib\_logfile0 文件。随着系统运行，Buffer Pool 的脏页刷新到了磁盘中， redo log 对应的记录没用了，会腾出空间记录新的更新操作。redo log 是循环写的方式相当于一个环形，用 write pos 表示 redo log 当前记录写到的位置，用 checkpoint 表示当前要擦除的位置。
...
系统默认的设置是 sync\_binlog \= 0，也就是不做任何强制性的磁盘刷新指令，这时候的性能是最好的，但是风险也是最大的，因为一旦主机发生异常重启，还没持久化到磁盘的数据就会丢失。

当 sync\_binlog 设置为 1 的时候，是最安全但是性能损耗最大的设置。因为当设置为 1 的时候，即使主机发生异常重启，最多丢失一个事务的 binlog，而已经持久化到磁盘的数据就不会有影响，不过就是对写入性能影响太大。
...
* 读提交在每个 select 都会生成一个新的 Read View，事务期间多次读取同一条数据，前后两次读的数据可能会出现不一致，因为可能这期间另外一个事务修改了该记录，并提交了事务。
* 可重复读隔离级别是**启动事务**时生成一个 Read View，然后整个事务期间都在用这个 Read View，这样就保证了在事务期间读到的数据都是事务启动前的记录。

通过 事务的 Read View 里的字段和记录中的两个隐藏列(trx\_id 和 roll\_pointer) 的比对，如果不满足可见行，就会顺着 undo log 版本链里找到满足其可见性的记录，从而控制并发事务访问同一个记录时的行为。
...
>
> 如果 sync\_binlog 和 当 innodb\_flush\_log\_at\_trx\_commit 都设置为 1，那么在每个事务提交过程中， 都会**至少调用 2 次刷盘操作**，一次是 redo log 刷盘，一次是 binlog 落盘，所以这会成为性能瓶颈。
...
1、主服务器上面的任何修改都会通过自己的 I/O tread(I/O 线程)保存在二进制日志 `Binary log`​ 也就是常说的 binlog 里面

2、从服务器上面也启动一个 I/O thread，通过配置好的用户名和密码, 连接到主服务器上面请求**读取二进制日志**，然后把读取到的二进制日志写到本地的一个`Relay log`​（中继日志）里面

3、从服务器上面同时开启一个 SQL thread 定时检查 `Relay log`​(这个文件也是二进制的)，如果发现有更新立即把更新的内容在本机的数据库上面执行一遍。

‍

![image](assets/image-20241124231925-7ael5wr.png)​
...
* 打开 MySQL 支持的==并行复制==，多个库并行复制。但如果说某个库的写入并发就是特别高，单库写并发达到了 2000/s，并行复制还是没意义。
...
分布式数据库 (OceanBase) 普及率较低，分库分表技术仍是大多数公司的选择

> 但是我有这个OB库的使用, 虽然不懂太多原理, 只是详细看过文档
...
看看你了解哪些分库分表的中间件，各个中间件的优缺点是啥？然后你用过哪些分库分表的中间件
...
* sharding-jdbc    当当开源的，属于client层方案。确实之前用的还比较多一些，因为SQL语法支持也比较多，没有太多限制，而且目前推出到了2.0版本，支持分库分表、读写分离、分布式id生成、柔性事务（最大努力送达型事务、TCC事务）。而且确实之前使用的公司会比较多一些（这个在官网有登记使用的公司，可以看到从2017年一直到现在，是不少公司在用的），目前社区也还一直在开发和维护，还算是比较活跃，个人认为算是一个现在也可以选择的方案。
  ...
  已经更名为 [`ShardingSphere`](https://github.com/apache/incubator-shardingsphere)​（后文所提到的 `Sharding-jdbc`​，等同于 `ShardingSphere`​）。确实之前用的还比较多一些，因为 SQL 语法支持也比较多，没有太多限制，而且截至 2019.4，已经推出到了 `4.0.0-RC1`​ 版本，支持分库分表、读写分离、分布式 id 生成、柔性事务（最大努力送达型事务、TCC 事务）。
...
#### MySQL分库分表如何在老业务上平滑上线分库分表？出现问题如何快速回滚？
...
* **双写机制/数据同步**  
  先在新架构下构建分库分表系统，同时保持老系统（单库单表）正常运行。通过双写或数据同步工具（例如利用ShardingSphere、MyCAT等中间件），让新增数据同时写入新旧两套系统。
* **逐步切流**  
  在充分验证数据一致性和查询正确性后，通过路由层（如代理或配置中心）逐步将读写流量切换到新系统，确保用户不感知业务中断。
...
* **保留老系统作为回退方案**  
  在平滑迁移期间，老系统依然在线，若新系统出现问题，可以迅速切换回老的单库单表系统。
* **监控与验证**  
  在上线过程中，必须实时监控数据路由、SQL执行和业务指标；一旦发现异常，立刻通过配置回滚流量，同时利用日志和数据校验工具迅速定位问题
...
### 分库分表如何查数据？以及对数据库索引的影响（比如按照用户id进行分表）
...
### MySQL分库分表选择哪个字段作为分片键？选择单个分片键还是复合分片键？
...
* **覆盖性与稳定性**  
  分片键应选取在绝大多数SQL中都能作为查询条件的字段，且该字段值在业务中较为稳定（例如用户ID、订单ID）。这样可以使得SQL能精确路由到目标分片，避免全库扫描。
* **数据分布均衡**  
  选取高基数字段有助于数据在各个分片中均匀分布，防止某些分片数据量过大导致热点问题。
...
* **单一分片键**
  ...
  * 优点：实现简单，配置和维护成本低。
  * 缺点：如果业务中有多种查询场景（例如同时涉及用户ID和订单ID的查询），可能无法完全覆盖所有路由需求
* **复合分片键**

  * 优点：可以结合多个字段的查询条件，使路由更精准，适用于查询场景复杂的业务。
  * 缺点：设计和算法实现复杂度较高，SQL编写和维护要求也相应增加。

‍

建议：在初期如果单一分片键能够满足绝大部分查询场景，优先使用；只有当业务场景要求在多维度上路由数据时，再考虑采用复合分片键
...
* **针对单个分片的进一步拆分（扩单表）**   
  当初步分库分表后，某个分片内的单表数据量仍然过大时，可考虑在该分片内部再进行细分（例如在原有分片键基础上加入范围分区或额外取模），实现单表的二次拆分，从而进一步降低每个物理表的数据量，提高查询性能。
* **整体扩容**  
  如果整个系统负载持续增长，除了针对热点表做细粒度拆分外，还可能需要增加新的数据库实例（新节点），并调整全局分片规则，实现水平扩展。
...
* **优先扩单表**  
  如果只有部分分片中的表数据量异常大，建议先对这部分表进行二次拆分，使每个物理表数据保持在合理范围。
* **系统整体扩展**  
  当整个系统数据量或并发压力全面上升时，再综合考虑扩展数据库节点和硬件资源，以应对整体性能瓶颈
...
​`DB_TRX_ID`​ 用来标识最近一次对本行记录做修改 (insert 、update) 的事务的标识符 ，即最后一次修改本行记录的事务 id

如果是 delete 操作， 在 InnoDB 存储引擎内部也属于一次 update 操作，即更新行中的一个特殊位 ，将行标识为己删除，并非真正删除。
...
从数据库的基本表中通过查询选取出来的数据组成的**虚拟表**（数据库中存放视图的定义）。可以对其进行增/删/改/查等操作。

视图是对若干张基本表的引用，一张虚表，查询语句执行的结果，不存储具体的数据（基本表数据发生了改变，视图也会跟着改变）；可以跟基本表一样，进行增删改查操作(ps:增删改操作有条件限制)；如连表查询产生的视图无法进行，对视图的增删改会影响原表的数据。
